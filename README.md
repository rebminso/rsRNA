...
```bash
    snakemake –cores 8 -q
```
The `--dryrun` or `-n` option allows you to see the scheduling plan including the assigned priorities. The main Snakefile in the `~./rsRNA/` directory will be automatically detected by snakemake and all the steps will be executed.
...
## Test run
The test files have been uploaded to this [link](https://drive.google.com/drive/folders/1Xb2dv9lqj7O3Kvewtx4qed-N6o5UR6wv?usp=share_link)

- `test/` directory is to be added in `/data/trimmed/` directory of rsRNA.
- RNA files to be added in `/data/Genome/`

Once deploying and configuring the `config/config.yaml` file as explained above under *usage* section, run the following command to run test sample:

```bash
  snakemake –cores 8 -q
```
In case of using another reference genome, 
